...
# ⚠⚠ 这一条第一版是**占位**(`med(...)` 与它自己比),我当时写了「占位」却**从未回来修** ——
#    它一路 PASS 到提交,直到 `#775` 的 lint 在 751 个脚本里把它扫出来。
#    ⇒ **一句写给自己的「占位」不是一个提醒,它是一条永远为真的检查。**
#    真正的检查在下面第二条(信度取 1 时校正必须是恒等变换),这一条删掉而不是修补:
#    **重复一条测同一件事的空检查,只会让汇总看起来更饱满。**
