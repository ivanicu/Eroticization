r"""#876 · E03·A102·R315 — the repair is applied; now measure the gap it cannot close
...
scripts = [f for f in sorted(ROOT.glob("E0*/**/*.py"))
           ...
           and not is_mine(str(f.relative_to(ROOT))) and IMPORTS.search(f.read_text(errors="replace"))]
if not scripts:
    raise SystemExit("STOP: an empty population must never be counted as a pass")
POP = f"the {len(scripts)} round scripts that import `round_path`, each executed alone from the repository root"
...
POS_OK = all(c == "IMPORT-ERROR" for c in before.values())
