...
|---|---|---|
| **① Variance composition** — what is this survey mostly made of? | **All three components equally [#85].** Referred each to its OWN null in one run: item **+0.208**, person×item interaction **+0.199**, person breadth **+0.148**. The ratio item:interaction is **1.0–2.4× across the rank sweep, 1.05× at each side's own best rank** — not the **11.7×** this row claimed for eighteen entries, which compared a corrected item effect against an interaction carrying a −0.19 estimator bias. Per block: item larger in 12/23, interaction larger in **7/23**, tied 4 | `#67` `#68` `#85` |
