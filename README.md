...
settled before anyone collects anything new. **163 numbered ledger entries** — [RETRACTIONS.md](RETRACTIONS.md)
...
| **It is a reliable personality dimension, and it is not "how many boxes you ticked"** | Split-half across disjoint block sets **+0.432** (floor −0.022, planted ceiling +0.832). **And the name itself is now tested** (`#163`): split the blocks into their two most content-dissimilar halves and **85% of the reliability survives**, while a pure *position* plant survives at 100% and a pure *content* plant collapses to −2% — **"prefers uncommon things" is a transferable position, not "likes one family that happens to be uncommon"**; **67%** survives removing the pick count, and its correlation with pick count (+0.608) is **lower** than the null's (+0.719) | `#100` `#104` |
...
**And this is a transferable methodological result, not one about this dataset only**: any group comparison on this release that is not matched on **block count** is measuring **survey coverage** for more than half of its effect rather than kink differences (`corr = +0.815` over nine splits; the matched correction takes pornhabit from 0.2285 → 0.0871 → 0.0439 and **reverses its ordering against sex**). ⚠ But "nobody has pointed this out" is **false, and `Entry 15` killed it** — the explorer's own `analysis/swarm/14-missingness.md`, **five months earlier**, states that group comparisons on gated columns compare different subpopulations. **The hazard is theirs; the quantification is mine.** (`#11` `#15`)
...
| file | what it holds |
|---|---|
| [`RETRACTIONS.md`](RETRACTIONS.md) | **163 entries.** Every claim killed, scoped or corrected, with what killed it. Twelve are a later round of mine destroying an earlier round of mine |
