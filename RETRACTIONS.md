...
**不是一轮实验。** 回答 Ivan「有哪些已经放出来的匿名化配置」时,去查了发布源,
**发现 Zenodo 记录里链着一个 Drive 文件夹,含  与 
—— 这个项目 400 轮从没读过。**

** ⚠⚠ 两个被人为粗化的变量正在已发表的结论里。**
卡片记录:**「你的成长环境有多性开放」原本 7 档,压成 3 档**;**关系风格原本 3 档,压成 2 档**。
**两个都是 (「什么都挡不住羞耻」,已上页面)的调节变量。**
**粗化一个调节变量会削弱交互的可探测性** —— 所以那个逐变量 MDE 10–30%
**有一部分是发布口径造成的,不是人群造成的**。已加页面注,验证:

** 发布方自述的关键数字,页面此前一句没写:**
- **相关比原始弱约 25%**(「base rates of most things are less affected」——**噪声主要打相关,不打边际**)
- 匿名化方式 = **相似行的人口学交换 + 全表噪声**(不是单纯加噪)
- **裁到 18–32 岁 · 只 US/CA/EU** ——  的「时长问不出来」**一半是人群被截,不是分箱**
- **(第一次性交的年龄)被整列删除** —— 起始年龄那条线最该有的锚,不在
- **全部恋童相关问题(门 + 子题)被移除**
- 全量清洗后是 **~980K 行 × ~797 列**;公开的是 15.5K × 365 -> **63× 行,2.1× 列**

** ⚠ 唯一一条数据质量警告,发布方写了而我从没记过:**
**「dirty talk 和 cunnilingus 两题可能是反的」。**
已查:两题**不在**我的 20 个结局里,全仓 **0 个脚本**引用 —— **没碰到任何结果。**
⚠ 但这正是 P15 的  那一列存在的理由:**一个复述了对象自身文档的结果是验证,不是发现。**

** 已放出的匿名化配置只有**一种**。**
Zenodo concept  下两个版本( 52.4MB /  52.2MB),
**描述逐字相同,同一条流水线**。本地文件 52,427,997 B = ,与 README 引用一致,**无版本混用**。
⚠ 卡片说 parquet 是 **15,511 × 376**,而我们的 CSV 是 **15,503 × 365** —— **两个不同的产物,待查,不猜。**

**一句关于人的话**:
没有 —— **但这一条说明:我把四百轮花在了一个我从没读过说明书的仪器上。**

**NEXT(两件,按顺序)**:
① **Production**:把「所有相关是下界(发布方自述 ~25%)」写进页面头段 ——
   它改变**每一个数**的读法,而现在页面对此完全沉默;
② 读完  的 ,
   **列出所有被重编码的量表**({0,1,2,3,5,8} → {0,1,2,3,4,5}),
   ⚠ 我的 Likert 检测用的是 {−3..3} 的值域 —— **先确认我筛出来的 20 个结局与它的重编码集一致**,
