...
KS=[4,5,6]; rows=[]
print(f"\n{'k':<4}{'n':>7}{'分半 r':>10}{'SB 信度':>10}{'置换零':>10}{'种植':>9}")
for k in KS:
    sd_=zlib.crc32(f'b{k}'.encode())%9973
    sb,r,n=rel_b(k,sd_); sbn,_,_=rel_b(k,sd_+1,perm=True); sbp,_,_=rel_b(k,sd_+2,plant=1.5)
    rows.append(dict(k=k,n=n,r=r,sb=sb,sb_null=sbn,sb_plant=sbp))
    print(f"{k:<4}{n:>7,}{r:>+10.4f}{sb:>+10.4f}{sbn:>+10.4f}{sbp:>+9.4f}",flush=True)
...
g.asserted('人内置换零在零附近',bool((T.sb_null.abs()<0.08).all()),
           " ".join(f"{v:+.3f}" for v in T.sb_null))
