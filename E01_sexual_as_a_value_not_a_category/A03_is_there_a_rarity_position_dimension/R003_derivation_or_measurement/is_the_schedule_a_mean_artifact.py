...
(A03/R071 第 66 行)。所以这个问题不是学术的:如果中位数排出的顺序不同,那么
...
SCOPE           报告 >=6 个类别起始年龄的人(与 A03/R071 同口径,便于直接比较)。
...
NOISE FLOOR     8 seeds(与 A03/R071 同)。
...
def acc(order_vals,people,rng,mode='skip_ties'):
    """⚠ mode='skip_ties' 是 A03/R071 的原做法,它**跳过**排序量并列的对。
