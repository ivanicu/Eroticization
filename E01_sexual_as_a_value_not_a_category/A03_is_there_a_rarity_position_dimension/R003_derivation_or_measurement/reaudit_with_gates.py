...
Three claims, re-read from their own committed artifacts -- no recomputation, so nothing can drift:
  ...
  #95   the rare-option signal is LICENSED           (A11/R139 grid)
  #99   the distribution is symmetrically WIDER      (A11/R146 grid)
  #100  the trait is RELIABLE and is not pick count  (A11/R147 grid)
...
D5=pd.read_csv(A/'R139_control_at_the_derived_magnitude/results/grid.csv',keep_default_na=False)
...
D12=pd.read_csv(A/'R146_is_it_elevation_or_just_width/results/grid.csv',keep_default_na=False)
...
B12=np.load(A/'R146_is_it_elevation_or_just_width/results/boot.npy')
...
D13=pd.read_csv(A/'R147_is_it_a_trait/results/grid.csv',keep_default_na=False)
