...
IDENTIFICATION  与 `A11/R14` **逐字相同**的构造,只把 `default_rng(8100)` 换成 10 个种子,
                并且**每个块用不同的种子**(原轮每块同种子,这本身是一个要记录的差异)。
SCOPE           A11/R14 的原口径(>=6 个已识别块的人)。
...
def cors(a):
    ...
    return out
real=cors(AR)
print(f"\n真实臂(复现 A11/R14):" + "  ".join(f"{k} {v:+.4f}" for k,v in real.items()),flush=True)
