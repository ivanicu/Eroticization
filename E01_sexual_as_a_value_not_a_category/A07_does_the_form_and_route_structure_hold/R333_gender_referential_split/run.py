...
"""
E01 A89 R333 -- 非不变性是集中在关于性别的题上,还是到处都是
...
def curve(rows,extra=None,only=None):
...
print(f"\n按结局子集分别做:")
ALL=arm(None,'全部 29 道(`#286a`)')
GENA=arm(GEN,f'性别指涉 {len(GEN)} 道')
NONA=arm(NONGEN,f'非性别指涉 {len(NONGEN)} 道')
HOLDA=arm(HOLD,f'★ 保留集 {len(HOLD)} 道')
r_sex,r_rnd,sd_rnd=HOLDA
...
T=pd.DataFrame([dict(arm='全部29',sex=ALL[0],rnd=ALL[1],sd=ALL[2]),
                dict(arm='性别指涉5',sex=GENA[0],rnd=GENA[1],sd=GENA[2]),
                dict(arm='非性别指涉24',sex=NONA[0],rnd=NONA[1],sd=NONA[2]),
                dict(arm='★保留集22',sex=HOLDA[0],rnd=HOLDA[1],sd=HOLDA[2])])
check_columns(T,'R331'); T.to_csv(pathlib.Path(__file__).parent/'results'/'joint_curves.csv',index=False)

g=Gate('非不变性在哪:性别指涉 vs 其余')
...
g.asserted('★ 注册的 kill(以保留集为准):非性别指涉结局上差塌到不可分辨 -> 只是几道题带性别;'
           '仍是 10× 量级 -> 结构真的是两套',
           abs(HOLDA[0]-HOLDA[1])<2*HOLDA[2],
           f"保留集 {HOLDA[0]-HOLDA[1]:+.4f}(2×展布 {2*HOLDA[2]:.4f})· "
           f"性别指涉 {GENA[0]-GENA[1]:+.4f} · 全部 {ALL[0]-ALL[1]:+.4f}")
print(g)
