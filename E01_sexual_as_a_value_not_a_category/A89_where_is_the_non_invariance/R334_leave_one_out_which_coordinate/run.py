...
"""
E01 A89 R334 -- 哪一个坐标带来了非不变性(留一)

`#288a`:非不变性是**整套结构**的(保留集 22 道上 −0.1172,3.0×),**但没说是哪一个坐标带来的**。
`#285b` 逐坐标比剖面的那一格因展布 0.4247 而不可用,
**而 `#286b` 证明联合层是稳的(展布 0.0197)—— 所以用留一**:
逐个去掉一个坐标,重算保留集上的「按性别劈 vs 随机劈」的差;
**去掉哪一个之后差最小,那一个就是主要来源。**
⚠ 每次留一都要**重跑随机劈的 offset**(展布会随坐标数变)。
...
def curve(rows,extra=None,only=None,drop=None,swap=None):
    Q=coords(rows)
    if swap is not None: Q=[(swap if i==swap_at else q_) for i,q_ in enumerate(Q)]
    if drop is not None: Q=[q_ for i,q_ in enumerate(Q) if i!=drop]
    Q=Q+([extra] if extra is not None else [])
...
def arm(only,tag):
    a,b=curve(g0,only=only),curve(g1,only=only); rs=sim(a,b)
    rr=[]
    for _ in range(4):
        p=rngS.permutation(np.flatnonzero(have))
        rr.append(sim(curve(p[:len(g0)],only=only),curve(p[len(g0):len(g0)+len(g1)],only=only)))
    rm,rd=float(np.mean(rr)),float(np.std(rr))
    print(f"  {tag:<26} 性别劈 **{rs:+.4f}** vs 随机劈 **{rm:+.4f} ± {rd:.4f}** -> "
          f"差 **{rs-rm:+.4f}**({abs(rs-rm)/max(2*rd,1e-9):.1f}× 的 2×展布)")
    return rs,rm,rd
print(f"两组 n = {len(g0):,} / {len(g1):,}")
def armd(drop,tag,swap=None):
    a,b=curve(g0,only=HOLD,drop=drop,swap=swap),curve(g1,only=HOLD,drop=drop,swap=swap)
    rs=sim(a,b); rr=[]
    for _ in range(4):
        p=rngS.permutation(np.flatnonzero(have))
        rr.append(sim(curve(p[:len(g0)],only=HOLD,drop=drop,swap=swap),
                      curve(p[len(g0):len(g0)+len(g1)],only=HOLD,drop=drop,swap=swap)))
    rm,rd=float(np.mean(rr)),float(np.std(rr))
    print(f"  {tag:<24} 性别劈 {rs:+.4f} vs 随机劈 {rm:+.4f} ± {rd:.4f} -> 差 **{rs-rm:+.4f}**"
          f"({abs(rs-rm)/max(2*rd,1e-9):.1f}× 的 2×展布)")
    return rs-rm,rd
print(f"\n留一(保留集 {len(HOLD)} 道;每次都重跑随机劈的 offset):")
base_d,base_sd=armd(None,'不去掉(基线)')
LOO=[]
for k,n in enumerate(NAMES):
    dd,ss=armd(k,f'去掉 {n}'); LOO.append((n,dd,ss))
best=max(LOO,key=lambda t:t[1])
print(f"\n**去掉之后差最小(最接近零)的是:{best[0]},差 {best[1]:+.4f}**"
      f"(基线 {base_d:+.4f});即它是非不变性的主要来源")
HOLDA=(0,0,base_sd); ALL=(0,0,0); GENA=(0,0,0); NONA=(0,0,0)
r_sex,r_rnd,sd_rnd=0,0,base_sd
...
def pc(extra):
    a=curve(g0,extra,only=HOLD); b=curve(g1,extra,only=HOLD); s_sex=sim(a,b)
    p=rngS.permutation(np.flatnonzero(have))
    s_rnd=sim(curve(p[:len(g0)],extra,only=HOLD),curve(p[len(g0):len(g0)+len(g1)],extra,only=HOLD))
    return s_sex-s_rnd
p1=pc(FAKE); p2=pc(INV)
print(f"\n正对照两端:① 加入已知按性别不同的量 -> 差 **{p1:+.4f}**(必须明显 <0)")
print(f"           ② 加入真实存在但几乎不随性别变的 `agreeableness` -> 差 **{p2:+.4f}**(必须 ≈0)")
swap_at=0
FAKE2=np.where(have,np.nan_to_num(SEX)*2.0+rngS.standard_normal(NN)*0.5,np.nan)
print(f"\n正对照:把**位置分 S** 换成一个按性别驱动的合成量,留一必须把它指认出来 ——")
sw_base,_=armd(None,'换掉后不去掉',swap=FAKE2)
sw=[]
for k,n in enumerate(NAMES):
    dd,_=armd(k,f'换掉后去掉 {n}',swap=FAKE2); sw.append((n,dd))
sw_best=max(sw,key=lambda t:t[1])
print(f"   -> 指认 **{sw_best[0]}**({'✅' if sw_best[0]==NAMES[swap_at] else '❌'};"
      f"被换掉的是 {NAMES[swap_at]})")
T=pd.DataFrame([dict(dropped=n,delta=dd,sd=ss) for n,dd,ss in LOO]
               +[dict(dropped='(基线)',delta=base_d,sd=base_sd)])
check_columns(T,'R331'); T.to_csv(pathlib.Path(__file__).parent/'results'/'joint_curves.csv',index=False)

g=Gate('哪一个坐标带来了非不变性(留一)')
g.asserted('⚠ 分组与污染声明已在跑之前冻结并独立提交',
           (pathlib.Path(__file__).parent/'PREREGISTRATION.md').exists(),'预注册文件存在')
g.asserted('正对照:把位置分 S 换成按性别驱动的合成量,留一必须指认出它',
           sw_best[0]==NAMES[swap_at], f"指认 {sw_best[0]};被换掉的是 {NAMES[swap_at]}")
g.asserted('★ 注册的 kill:去掉某一个坐标后差明显缩小 -> 它是主要来源;都不缩小 -> 非不变性是分散的',
           best[1]>base_d+2*base_sd,
           f"基线 {base_d:+.4f};去掉 {best[0]} 后 {best[1]:+.4f}(2×展布 {2*base_sd:.4f});"
           +' · '.join(f"{n} {dd:+.3f}" for n,dd,_ in LOO))
print(g)
