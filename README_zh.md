...
| **两个正交的人层维度,各自指向不同的东西** | 把**内容**分(你勾了哪些具体选项)对**位置**分(它们有多冷门)回归掉,两者一起跑遍 20 道 Likert 结局、各用最大统计量零给全族阈值。**位置侧 9/20 越阈值,最大的是羞耻(+0.1155)**;**内容残差侧 6/20 越阈值,最大的是 `animated`(−0.1063,8.7×)与 `written`(−0.0630)**,还有 `biomale`(+0.0707)与两道性别身份题 —— **而它完全不预测羞耻**(+0.0029,0.2×)。所以**你的口味有多冷门**贴着**羞耻**,**你具体喜欢哪些**贴着**媒介**(画的还是写的)与**性别身份**。⚠ **这个分工有一部分是控制勾选数造出来的**:**边际**评估下位置分对 `animated` = **+0.1193(10.0×)**,与它对羞耻的 **+0.1185** 在同一把尺上不可分 —— 干净的分工只在**给定勾选数**之后才出现(`#192`)。⚠ **正交性是构造出来的,而原始的两个维度根本不正交**:人层 `corr(S, C) = +0.4323`,所以 `corr(S, C残差) = 0` 是定义不是发现(`#129` `#189d`)—— 它们是**一个维度、和另一个扣掉它之后的余量**,不是两个独立的东西,而内容分是跨人主成分,完全特异的组合对它不可见 | `#188` `#189` `#192` |
